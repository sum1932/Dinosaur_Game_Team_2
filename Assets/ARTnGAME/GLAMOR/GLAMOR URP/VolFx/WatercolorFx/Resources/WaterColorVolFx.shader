...
#define SAMPLES_INT 16
	...
	{
		return _MainTexA.Sample(sampler_MainTexA, p).rgb * 2;
	}
	...
	float4 FragWaterColor(v2fWC i) : SV_Target
	{
		//return float4(1,0,0,1);
		//return _NoiseTex.Sample(sampler_NoiseTex, i.uv).rgba * 1.2;
		//return _MainTexA.Sample(sampler_MainTexA, i.uv).rgba * 1.2;
		...
		///return float4(Process(i.uv), 1);
		return float4(Process(i.uv), 1) * (_Blend) + (1-_Blend) * _MainTexA.Sample(sampler_MainTexA, i.uv);
	}
			 ...
			 v2f vert(VertexData v) {
				...
				// 8: Mask
						...
						Pass
					{
						...
						{
							return _MainTexA.Sample(sampler_MainTexA, pos / _ScreenParams.xy);
						}
						...
						float4 Frag(v2fA i) : SV_Target
						{
							float4 original = _MainTexA.Sample(sampler_MainTexA,i.uv.xy); /// SAMPLER2D(_MainTexA, sampler_MainTexA, i.uv);// getCol(i.uv.xy);// float4(1,0,0,1);
							...
							float4 outME = lerp(float4(0.0, 0.0, 0.0, 0.0), background, weight) * (_blendThreshold)+(original * (_blendScreenThreshold));
							
							// +_MainTexA.Sample(sampler_MainTexA, pos / _ScreenParams.xy);

							return outME * _Blend + original * (1 - _Blend);
						}
						ENDHLSL
					}

					Pass //PASS 13 Watercolor
					{
						HLSLPROGRAM
						...
						#pragma vertex VertWC


						#pragma fragment FragWaterColor
						ENDHLSL
					}
