	...
	SubShader
	{
		...
		/*	#include "UnityCG.cginc"
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float4 col = SAMPLE_TEXTURE2D(_MainTexA,sampler_MainTexA, i.uv) + _Blend*SAMPLE_TEXTURE2D(_MainTexB,sampler_MainTexB, i.uv);
				return col;
			}
			ENDHLSL
		}

				//PASS6 TEST STAGE //PASS 7
				Pass{
						ZTest Always Cull Off ZWrite Off
